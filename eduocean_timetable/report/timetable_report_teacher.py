...
class ReportTimeTableTeacherGenerate(models.AbstractModel):
    _name = 'report.eduocean_timetable.report_timetable_teacher_generate'

    def get_full_name(self, data):
        teacher_name = self.env['op.teacher'].browse(data['teacher_id'][0])
        return ' '.join([teacher_name.name,
                         teacher_name.middle_name or '',
    ...
    def render_html(self, docids, data=None):
        ...
        return self.env['report'] \
            .render('eduocean_timetable.report_timetable_teacher_generate',
                    docargs)
